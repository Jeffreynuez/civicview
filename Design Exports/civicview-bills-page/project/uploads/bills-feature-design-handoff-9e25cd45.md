...
- **Seat shape + size:** rounded square (`--cl-radius-xs`). **Standard size**
  (Jeffrey's pick — not compact or large). (Hex cartogram is P2 / out of scope.)
...
> **Real-data note:** the prototype shows initials + a generic "Representative ·
> ST" because it uses seeded data. In production these are real member names —
> initials is a deliberate density choice for this dense list; search + the
> mini-card cover finding a specific person.
...
|---|---|---|
| **Desktop ≥1025px** | Full interactive hemicycle, standard seat size + state label under each seat | Two columns, below chart |
| **Tablet 901–1024px** | Hemicycle scales via container-query off panel width; seats stay standard, labels kept | Two columns, below chart |
...
2. **Seat size + ordering** → **standard** size; **party-blocked, state-sorted
   within block**; per-seat 2-letter state label; Senate rebuilt into tidy rows.
