...
Guards (added 2026-09-25, audit E13):
  * Florida is refused outright. data/fl/ is curated by hand from the
    Florida Division of Elections (certified primary results, verified
    nominees, withdrawals, write-ins); this script would replace all of
    that with an unverified OpenFEC filer list.
  * --write will not replace existing files unless --overwrite is also
    given, because every state's files have since been corrected by
    hand (for example duplicate candidate records in Texas and invented
    ballot measures in New York). After a regeneration, run
    tests/test_data_lint.py and re-apply those corrections.

Usage:
  python scripts/build_state_federal_candidates.py --state TX            # dry run
  python scripts/build_state_federal_candidates.py --state TX --write    # new state only
  python scripts/build_state_federal_candidates.py --state TX --write --overwrite
"""
...
# States whose election files are maintained by hand and must never be
...
STATE_BALLOT_NOTE = {
    ...
    "NY": ("New York's statewide ballot proposals for the November 2026 general "
           "election are not yet integrated, pending the New York State Board of "
           "Elections' certified list. Local and New York City ballot proposals are "
           "not yet integrated either."),
...
STATE_BALLOT_MEASURES = {
    # New York's two entries were removed on 2026-09-25: they were the
    # November 2023 proposals (small city school district debt, sewage
    # facility debt), which voters approved that year, not 2026 measures.
}
...
async def main():
    ...
    ap.add_argument("--overwrite", action="store_true",
                    help="with --write, replace existing files (hand corrections are lost)")
    args = ap.parse_args()

    if args.state.upper() in CURATED_STATES:
        print(f"ERROR: {args.state.upper()} election data is curated by hand; this script "
              "must not regenerate it. See the Guards section in this file's docstring.",
              file=sys.stderr)
        sys.exit(2)
    out_dir = DATA_DIR / args.state.lower()
    existing = [p for p in (out_dir / "candidates.json", out_dir / "elections.json") if p.exists()]
    if args.write and existing and not args.overwrite:
        print("ERROR: refusing to replace " + ", ".join(str(p) for p in existing) +
              ". These files carry hand corrections. Pass --overwrite if you really "
              "mean to regenerate them, then run tests/test_data_lint.py.", file=sys.stderr)
        sys.exit(2)
